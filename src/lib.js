...
function getuserCookie() {
  let MMUserId = Cookies.get("MMUserId");
  if (!MMUserId) {
    const uuid = uuidv4();
    Cookies.set("MMUserId", uuid);
    MMUserId = uuid;
  }
...
function calculatePercDiscount(price, wasprice) {
  try {
    const discperc = Number((((wasprice - price) / wasprice) * 100).toFixed(0));
    return discperc + "%";
  } catch {
    return 0;
  }
