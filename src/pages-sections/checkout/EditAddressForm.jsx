...
const EditAddressForm = (props) => {
  ...
  const initialValues = {
    name: addressData[0].OCM_BTO_FIRST_NAME,
    phone: addressData[0].OCM_MAIN_PHONE,
    address1: addressData[0].OCM_BTO_ADDRESS1,
    address2: addressData[0].OCM_BTO_CITY,
  };
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const updated = addressData.map((item) => {
        return item.OCM_DOCNO === addressData[0].OCM_DOCNO ? values : item;
      });
